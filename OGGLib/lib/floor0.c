...
static void *floor0_inverse1(vorbis_block *vb,vorbis_look_floor *i){
  ...
  if(ampraw>0){ /* also handles the -1 out of data case */
    unsigned long maxval=(1<<info->ampbits)-1;
#ifndef FIXED_POINT
    float amp=(float)ampraw/maxval*info->ampdB;
#else
    /* FIXP note: 4 fracbits for amp */
    FIXP amp=(ampraw<<4)*info->ampdB/maxval;
#endif
    ...
    if(booknum!=-1 && booknum<info->numbooks){ /* be paranoid */
      ...
      FIXP last=0;
      FIXP *lsp=_vorbis_block_alloc(vb,sizeof(*lsp)*(look->m+1));
...
vorbis_func_floor floor0_exportbundle={
  NULL,&floor0_unpack,&floor0_look,NULL,&floor0_free_info,
  &floor0_free_look,NULL,&floor0_inverse1,&floor0_inverse2
};
