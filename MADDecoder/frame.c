...
int free_bitrate(struct mad_stream *stream, struct mad_header const *header)
{
  ...
  slots_per_frame = ((header->flags & MAD_FLAG_LSF_EXT)) ? 72 : 144;
 ...
 */
static __inline int mad_header_decode(struct mad_header *header, struct mad_stream *stream)
{
...
//		32 * MAD_NSBSAMPLES(header), header->samplerate);
  ...
  if (header->bitrate == 0||(header->samplerate==0)) goto fail;
